...
// The provider sits on the user's own hardware and the server proxies through
// it, so these calls deliberately do not inherit the short global timeout.
...
export const mapsApi = {
  search: (query: string, _lang?: string) =>
    apiClient.post(`/maps/search?lang=${MAPS_CONTENT_LANGUAGE}`, { query }).then(r => checkInDev(mapsSearchResultSchema, r.data, 'maps.search')),
  autocomplete: (
    input: string,
    _lang?: string,
    ...
    signal?: AbortSignal,
    sessionToken?: string,
  ) => apiClient.post('/maps/autocomplete', { input, lang: MAPS_CONTENT_LANGUAGE, locationBias, sessionToken }, { signal })
    .then(r => checkInDev(mapsAutocompleteResultSchema, r.data, 'maps.autocomplete')),
  details: (placeId: string, _lang?: string, sessionToken?: string) =>
    apiClient.get(`/maps/details/${encodeURIComponent(placeId)}`, { params: { lang: MAPS_CONTENT_LANGUAGE, sessionToken } })
      .then(r => checkInDev(mapsPlaceDetailsResultSchema, r.data, 'maps.details')),
  // Pictures describe a place the user looked at but has not yet saved. Fans out
  // to several providers server-side, so the caller is expected to abort when
  // selection changes; it also gets a longer timeout than the global 8s default.
  placeEnrichment: (
    body: { placeId?: string; lat: number; lng: number; name: string; lang?: string; details?: Record<string, unknown> },
    signal?: AbortSignal,
  ) => apiClient.post('/maps/enrichment', { ...body, lang: MAPS_CONTENT_LANGUAGE }, { signal, timeout: 25000 })
    .then(r => checkInDev(mapsPlaceEnrichmentResultSchema, r.data, 'maps.placeEnrichment')),
  // Author and licence for a picture already stored in the photo cache, keyed by
  // the cache key embedded in its proxy URL. This is a local read, no provider call.
  ...
  placePhoto: (placeId: string, lat?: number, lng?: number, name?: string) =>
    apiClient.get(`/maps/place-photo/${encodeURIComponent(placeId)}`, { params: { lat, lng, name } })
      .then(r => checkInDev(mapsPlacePhotoResultSchema, r.data, 'maps.placePhoto')),
  reverse: (lat: number, lng: number, _lang?: string) =>
    apiClient.get('/maps/reverse', { params: { lat, lng, lang: MAPS_CONTENT_LANGUAGE } })
  ...
  // timeout instead of aborting at 8s and showing nothing.
  pois: (category: string, bbox: { south: number; west: number; north: number; east: number }, lang?: string, signal?: AbortSignal) =>
    apiClient.get('/maps/pois', { params: { category, ...bbox, lang }, signal, timeout: 20000 })
