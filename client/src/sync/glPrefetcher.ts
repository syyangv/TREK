...
import { computeBbox, latToTileY, lngToTileX, type TileBbox } from './tilePrefetcher';
...
/** Workbox runtime cache for vector assets and tiles (see vite.config.js). */
const VECTOR_CACHE = 'gl-map-offline';

/** Workbox runtime cache for style documents (see vite.config.js). */
const STYLE_CACHE = 'gl-map-styles';
...
async function warm(cache: Cache | null, url: string): Promise<boolean> {
  try {
    // Returning true after a successful network request when `cache` is null
    // would make callers report an offline download that cannot be replayed.
...
async function openVectorCache(): Promise<Cache | null> {
  return openCache(VECTOR_CACHE);
}

async function openStyleCache(): Promise<Cache | null> {
  return openCache(STYLE_CACHE);
}
...
export async function prefetchStyleAssets(styleUrl: string): Promise<string | null> {
  if (!hasControllingServiceWorker() || !navigator.onLine) return null;

  const [styleCache, vectorCache] = await Promise.all([openStyleCache(), openVectorCache()]);
  // Both caches are required: the style document is kept separate so the
  // Workbox tile LRU cannot evict it, while sprites/glyphs/TileJSON belong with
  // the vector tile cache. No cache means no offline completion.
  if (!styleCache || !vectorCache) return null;

  const style = await fetchJson<StyleDoc>(styleUrl);
  if (!style) return null;
  await warm(styleCache, styleUrl);

  if (style.sprite) {
    // Both densities, both parts: a style asks for whichever matches the screen.
    await Promise.all([
      warm(vectorCache, `${style.sprite}.json`),
      warm(vectorCache, `${style.sprite}.png`),
      warm(vectorCache, `${style.sprite}@2x.json`),
      warm(vectorCache, `${style.sprite}@2x.png`),
    ]);
  }
    ...
    await Promise.all(
      stacks.flatMap((stack) =>
        GLYPH_RANGES.map((range) =>
          warm(vectorCache, style.glyphs!.replace('{fontstack}', encodeURIComponent(stack)).replace('{range}', range))
        )
      )
    );
  }
  ...
  for (const source of Object.values(style.sources ?? {})) {
    ...
    if (source.url) {
      await warm(vectorCache, source.url);
...
export async function prefetchVectorForPlaces(
  ...
  if (!navigator.onLine || !hasControllingServiceWorker()) return { tiles: 0, template: null };
  ...
  if (!cache) return { tiles: 0, template: null };
...
export async function clearVectorCache(): Promise<void> {
  try {
    ...
    await Promise.all([caches.delete(VECTOR_CACHE), caches.delete(STYLE_CACHE)]);
  } catch {
