...
import { attributionForTile, OFM_ATTRIBUTION, OFM_POSITRON } from '../constants/mapDefaults';
...
const STATIC_RASTER = 'https://tiles.example.test/tile.png';
...
describe('isVectorStyle', () => {
  it('FE-UTIL-BASEMAP-001: a style document is one, a tile template is not', () => {
    ...
    expect(isVectorStyle(STATIC_RASTER)).toBe(false);
...
describe('resolveTileUrl and the retired CARTO basemaps', () => {
  it('FE-UTIL-BASEMAP-002: a keyless CARTO template falls back to the default', () => {
    ...
    expect(resolveTileUrl('https://basemaps.cartocdn.com/rastertiles/voyager/{z}/{x}/{y}.png', OFM_POSITRON)).toBe(
      OFM_POSITRON
    );
  });
...
describe('resolveBasemap', () => {
  ...
  it('FE-UTIL-BASEMAP-010: a legacy static image URL is not treated as a style document', () => {
    expect(resolveBasemap(STATIC_RASTER, OFM_POSITRON)).toEqual({ kind: 'raster', url: STATIC_RASTER });
  });
