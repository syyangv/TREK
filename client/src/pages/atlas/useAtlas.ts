...
export function useAtlas() {
  ...
  useEffect(() => {
    ...
    // Cancellation belongs to this effect invocation. Reusing a ref here lets
    // a newer map reset the flag while an older async MapLibre import is still
    // resolving, so the stale layer can attach to the removed map.
    let cancelled = false;
    ...
    if (isVectorStyle(tileUrlRef.current)) {
      void attachVectorBasemap(map, tileUrlRef.current, glLayerRef, () => cancelled, { hideLabels: true });
    } else {
