...
describe('usePackingList — grouping, filtering and progress', () => {
  ...
  const withCompanion = [{ id: 1, username: 'owner' }, { id: 2, username: 'friend' }]

  it('FE-W5HOOK-008: the common view hides private items and buckets by category', async () => {
    const { result } = renderPanel({ items, view: 'common', tripMembers: withCompanion })
    await settled()
  ...
  it('FE-W5HOOK-009: the personal view shows only private items', async () => {
    const { result } = renderPanel({ items, view: 'personal', tripMembers: withCompanion })
    await settled()
  ...
  it('FE-W5HOOK-014: a controlled view prop drives the split and reports changes upwards', async () => {
    ...
    const { result } = renderPanel({ items, view: 'personal', onViewChange, tripMembers: withCompanion })
    await settled()
...
describe('usePackingList — item and category CRUD', () => {
  ...
  it('FE-W5HOOK-022: a duplicate category name gets a zero-width suffix so it stays distinct', async () => {
    ...
    const { result } = renderPanel({ items: [buildPackingItem({ id: 20, category: 'Gear' })], view: 'common', tripMembers: [{ id: 1, username: 'owner' }, { id: 2, username: 'friend' }] })
    await settled()
