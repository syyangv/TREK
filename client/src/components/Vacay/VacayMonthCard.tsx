...
export default function VacayMonthCard({
  year, month, holidays, companyHolidaySet, companyHolidayColorMap, companyHolidaysEnabled = true, entryMap, sharedMap,
    ...
    <div className="vg-card rounded-[22px]" style={{ padding: '15px 16px 14px' }}>
      ...
      <div className="grid grid-cols-7" style={{ gap: 4 }}>
        {weeks.flat().map((day, di) => {
          ...
          else if (dayEntries.length === 0 && isCompany) background = `color-mix(in srgb, ${companyHolidayColorMap?.get(dateStr) || '#f59e0b'} 22%, transparent)`
