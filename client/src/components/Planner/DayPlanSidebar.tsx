...
interface DayPlanSidebarProps {
  ...
  /** Seed the mobile-sheet instance from the desktop planner's expansion state. */
  initialExpandedDayIds?: Set<number> | null
  isMobile?: boolean
  /** Coarse pointer: give the persistent time-slot control a real tap target. */
  isTouch?: boolean
}
...
{canEditDays && place && (
                              <button type="button" onClick={e => {
                                e.stopPropagation()
                                setTimeSlotEdit({ dayId: day.id, assignmentId: assignment.id, place_time: (place.place_time || '').substring(0, 5), end_time: (place.end_time || '').substring(0, 5) })
                              }} title={t('dayplan.timeSlot')} aria-label={t('dayplan.timeSlot')}
                              style={{ background: 'transparent', border: 'none', color: 'var(--text-faint)', width: 44, height: 44, minWidth: 44, minHeight: 44, padding: 0, display: 'grid', placeItems: 'center', cursor: 'pointer', touchAction: 'manipulation' }}
                              onMouseEnter={e => { e.currentTarget.style.color = 'var(--text-primary)' }}
                              onMouseLeave={e => { e.currentTarget.style.color = 'var(--text-faint)' }}>
