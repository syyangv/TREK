...
export function useStorageAdmin(genericError: string, conflictError: string): StorageAdmin {
  ...
  // Separate request generation: stateSeq only advances when a response is
  // applied, so overlapping GETs otherwise share one generation and an older
  // response can overwrite the newer state.
  ...
  const refreshState = useCallback(async (): Promise<void> => {
    ...
    if (refreshRequestSeq.current !== requestSeq || stateSeq.current !== seq || savingRef.current) return;
    stateSeq.current += 1;
    stateRef.current = next;
    setState(next);
  ...
  useEffect(() => {
    if (!state?.backfills.some((b) => b.status === 'running') && !state?.migrations.some((m) => m.status === 'running'))
      return;
