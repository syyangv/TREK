...
function JourneyMap({
  ...
  useEffect(() => {
    ...
    // Keep cancellation scoped to this map instance. A shared ref can be reset
    // by a newer effect before an older async MapLibre import resolves, allowing
    // the stale layer to attach to a map that has already been removed.
    let cancelled = false;
    ...
    if (isVectorStyle(tileUrlRef.current)) {
      void attachVectorBasemap(map, tileUrlRef.current, glLayerRef, () => cancelled);
    } else {
