...
describe('MapViewGL', () => {
  ...
  it('FE-COMP-MAPVIEWGL-071: long-pressing a custom MapLibre place pin does not open Add-Place', async () => {
    vi.useFakeTimers();
    try {
      glMap.on.mockImplementation((event: string, handlerOrLayer: unknown) => {
        if (event === 'load' && typeof handlerOrLayer === 'function') (handlerOrLayer as () => void)();
      ...
      render(
        <MapViewGL
          places={[buildMapPlace({ id: 1, lat: 3, lng: 2 })]}
          fitKey={1}
          glProvider="maplibre-gl"
          onMapContextMenu={onContext}
        />
      );
      await act(async () => {});

      const pin = glCanvasContainer.querySelector('[data-trek-map-marker]') as HTMLElement | null;
      expect(pin).toBeTruthy();
      act(() => {
        pin!.dispatchEvent(touchEvent('touchstart', [{ clientX: 30, clientY: 40 }]));
