...
vi.mock('react-leaflet', () => ({
  useMap: () => leafletMap,
  TileLayer: ({ url }: { url: string }) => createElement('div', { 'data-testid': 'vector-fallback', 'data-url': url }),
}));
...
describe('VectorBasemap', () => {
  ...
  it('FE-COMP-VECBM-013: falls back to a keyless raster when the vector renderer fails', async () => {
    maplibreGL.mockImplementationOnce(() => {
      throw new Error('WebGL unavailable');
    });
    const view = render(<VectorBasemap style={OFM_POSITRON} />);

    const fallback = await view.findByTestId('vector-fallback');
    expect(fallback).toHaveAttribute('data-url', expect.stringContaining('tile.openstreetmap.org'));
  });
});
