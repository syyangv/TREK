...
import type { Accommodation, RouteResult, RouteSegment, RouteVia } from '../types';
import { getMergedItems, getTransportForDay, getTransportRouteEndpoints, isCarrierTransport } from '../utils/dayMerge';
import { getDayBookendHotels, shouldDrawEveningLeg, shouldDrawMorningLeg, type CarrierEdge } from '../utils/dayOrder';
...
export function useRouteCalculation(
  ...
  const selectedDayDefaultMode = useTripStore((s) =>
    selectedDayId ? (s.days?.find((d) => d.id === selectedDayId)?.default_transport_mode ?? null) : null
  );
  ...
  const updateRouteForDay = useCallback(
    async (dayId: number | null) => {
      ...
      for (const entry of entries) {
        if (entry.kind === 'place') {
          ...
          currentRun.push({
            ...
            lng: entry.lng,
            isPlace: true,
            leg_transport_mode: entry.mode,
            incoming_leg_transport_mode: entry.incoming,
          });
          runHasPlace = true;
        } else if (entry.from || entry.to) {
          const prev = currentRun[currentRun.length - 1];
          if (entry.from && (!prev || withinDriveRange(prev, entry.from)))
            currentRun.push({ ...entry.from, isPlace: false });
          closeRun();
      ...
      for (const e of entries) {
        if (e.kind === 'place')
          flatPts.push({
            ...
            lng: e.lng,
            isPlace: true,
            leg_transport_mode: e.mode,
            incoming_leg_transport_mode: e.incoming,
          });
        else {
          if (e.from) flatPts.push({ ...e.from, isPlace: false });
          if (e.to) flatPts.push({ ...e.to, isPlace: false });
        }
      }
      ...
      const edgeInfo = (e: Entry | undefined, side: 'first' | 'last') => {
        ...
        const role: CarrierEdge =
          side === 'first' ? (e.from ? 'departure' : 'arrival') : e.to ? 'arrival' : 'departure';
      ...
      const drawMorning =
        !!bookends &&
        !!day &&
        shouldDrawMorningLeg(bookends, day, edgeInfo(firstStop, 'first')) &&
        (!morningHotel || !firstWay || firstWay.isPlace || withinDriveRange(morningHotel, firstWay));
      const drawEvening =
        !!bookends &&
        !!day &&
        shouldDrawEveningLeg(bookends, day, edgeInfo(lastStop, 'last')) &&
          ...
          while (i < run.length - 1) {
            ...
            try {
              const r = await calculateRouteWithLegs(
                ...
                { signal: controller.signal, profile: mode, tripId, dayId }
              );
    ...
    return reservationsForSignature
      ...
      .map((r) => {
        ...
        const eps = (r.endpoints || [])
          .map(
            (e) =>
              `${e.role}@${e.lat ?? ''},${e.lng ?? ''}@${e.local_date ?? ''},${e.local_time ?? ''}@${e.timezone ?? ''}`
          )
          .join(';');
        const metadata = typeof r.metadata === 'string' ? r.metadata : JSON.stringify(r.metadata ?? null);
        return `${r.id}:${r.type}:${r.day_id ?? ''}:${r.end_day_id ?? ''}:${r.reservation_time ?? ''}:${r.reservation_end_time ?? ''}:${pos ?? ''}:${eps}:${metadata ?? ''}`;
      })
