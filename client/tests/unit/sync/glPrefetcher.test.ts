...
import { clearVectorCache, prefetchStyleAssets, prefetchVectorForPlaces } from '../../../src/sync/glPrefetcher';
...
describe('prefetchStyleAssets', () => {
  it('does not prefetch when there is no controlling service worker', async () => {
    Object.defineProperty(navigator, 'serviceWorker', {
  ...
  it('does not claim completion when Cache Storage cannot be opened', async () => {
    vi.stubGlobal('caches', {
  ...
  it('FE-SYNC-GLPF-001: takes the tile template from the TileJSON rather than guessing it', async () => {
    ...
    expect(opened).toEqual(expect.arrayContaining(['gl-map-styles', 'gl-map-offline']));
  });
...
describe('clearVectorCache', () => {
  it('FE-SYNC-GLPF-012: drops the offline vector basemap', async () => {
    ...
    expect(deleted).toEqual(['gl-map-offline', 'gl-map-styles']);
  });
