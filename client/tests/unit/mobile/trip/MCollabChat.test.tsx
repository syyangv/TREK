...
describe('MCollabChat', () => {
  ...
  it('FE-MOB-CCHAT-030: auto-scrolls to a remote message only while the view sits at the bottom', async () => {
    ...
    // Wait for the initial "jump to newest" pass rather than racing its
    // timer; the remote-message assertion below must only observe new work.
    await waitFor(() => expect(Element.prototype.scrollTo).toHaveBeenCalledWith({ top: expect.any(Number), behavior: 'auto' }))
