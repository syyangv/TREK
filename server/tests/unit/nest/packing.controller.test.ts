...
describe('PackingController (parity with the legacy /api/trips/:tripId/packing route)', () => {
  ...
  describe('templates', () => {
    ...
    it('passes a personal category scope when saving one list as a template', () => {
      ...
      const svc = makeService({ saveAsTemplate } as Partial<PackingService>);
      new PackingController(svc).saveAsTemplate(admin, '5', { name: 'Before departure', category: 'Before departure', visibility: 'personal' });
    ...
    it('returns 409 with a stable error code when the template name already exists', () => {
      ...
      expect(thrown(() => new PackingController(svc).saveAsTemplate(admin, '5', { name: 'Before departure' }))).toEqual({
        status: 409,
    ...
    it('forwards overwrite when replacing a category template', () => {
      ...
      const svc = makeService({ saveAsTemplate } as Partial<PackingService>);
      new PackingController(svc).saveAsTemplate(admin, '5', { name: 'Before departure', category: 'Before departure', visibility: 'personal', overwrite: true });
      expect(saveAsTemplate).toHaveBeenCalledWith('5', admin.id, 'Before departure', {
        category: 'Before departure', visibility: 'personal',
      }, true);
    });
