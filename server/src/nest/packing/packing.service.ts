...
export class PackingService {
  ...
  saveAsTemplate(
    ...
    if (scope?.visibility === 'personal') {
      items = this.db.all<{ name: string; category: string | null }>(`
      ...
      `, tripId, scope.category, userId, userId)
    } else if (scope) {
      items = this.db.all<{ name: string; category: string | null }>(`
        SELECT name, category FROM packing_items
        WHERE trip_id = ? AND category = ? AND COALESCE(is_private, 0) = 0
        ORDER BY sort_order ASC
      `, tripId, scope.category)
    } else {
      // A template is a durable artifact. Capture the common pool plus the
      // actor's own private items, but never another member's private data.
      items = this.db.all<{ name: string; category: string | null }>(
    ...
    const existing = this.db.get<{ id: number; name: string }>(
    ...
    return this.db.transaction(() => {
      ...
      if (existing) {
        ...
        this.db.run('DELETE FROM packing_template_categories WHERE template_id = ?', templateId)
        this.db.run('UPDATE packing_templates SET name = ?, created_by = ? WHERE id = ?', templateName, userId, templateId)
      } else {
        templateId = this.db.run('INSERT INTO packing_templates (name, created_by) VALUES (?, ?)', templateName, userId).lastInsertRowid
      }
