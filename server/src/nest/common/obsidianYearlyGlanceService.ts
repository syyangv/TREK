...
export function loadObsidianPublicHolidaysForYear(year: number): ObsidianHoliday[] {
  ...
  // Yearly Glance treats the daily note as the actual record for that date;
  // the leave-plan table is the fallback for planned dates.  Do not import
  // arbitrary Yearly Glance custom events: they are not leave declarations.
  const holidayByDate = new Map(leavePlanHolidays(vaultPath, year).map(holiday => [holiday.date, holiday]));
  ...
  for (let offset = 0; offset < days; offset++) {
    ...
    // A Daily Note is the actual source and wins over a planned row.  An
    // existing note with an empty/invalid 假期 value also suppresses a stale
    // plan row, matching Yearly Glance's note-governs-date merge rule.
    if (note) holidayByDate.set(dateStr, { date: dateStr, note });
    else holidayByDate.delete(dateStr);
  }
