...
export class SynologyService {
  ...
  async getSynologyAlbumPhotos(
    ...
    // Shared albums use a passphrase, but the album-list response must not expose
    // that credential to MCP clients. Resolve it inside the provider service when
    // the caller did not supply one; personal albums continue through the
    // album_id path when no passphrase exists.
    let effectivePassphrase = passphrase;
    if (!effectivePassphrase) {
      const listed = await this.listSynologyAlbums(userId);
      if (listed.success) {
        effectivePassphrase = listed.data.albums.find((album) => album.id === String(albumId))?.passphrase;
      }
    }
    ...
    while (true) {
      const params: ApiCallParams = effectivePassphrase
