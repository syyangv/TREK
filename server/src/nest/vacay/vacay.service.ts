...
} from '../common/obsidianYearlyGlanceService';
  ...
  async updatePlan(planId: number, body: UpdatePlanBody, socketId: string | undefined) {
    ...
    if (carry_over_enabled === true) {
      ...
      this.db.transaction(() => {
        ...
        `, u.id, planId, nextYr, vacationDays, carry, carry);
          }
        }
      });
    }
    ...
    if (!exists) {
      this.db.transaction(() => {
        ...
        for (const u of users) {
          ...
          if (carryOverEnabled && prevConfig) {
          ...
          this.db.run('INSERT OR IGNORE INTO vacay_user_years (user_id, plan_id, year, vacation_days, carried_over) VALUES (?, ?, ?, ?, ?)', u.id, planId, year, vacationDays, carriedOver);
        }
      });
    }
    this.notifyPlanUsers(planId, socketId, 'vacay:settings');
