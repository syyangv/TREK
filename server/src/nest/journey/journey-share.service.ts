...
export class JourneyShareService {
  ...
  getPublicJourney(token: string) {
    ...
    // Only explicitly shared entries cross an anonymous share link. `private`
    // is the default for journal entries and must remain private even when the
    // owner enables the timeline or map for the journey as a whole.
