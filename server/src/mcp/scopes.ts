 ...
 *
 * `files:content` hands a client the bytes of uploaded documents, and
 * `plugins:use` runs third-party code as the caller. Neither should be granted
 * implicitly, so both are excluded from the DCR fallback below and from the
 * client-side presets. Asking for either explicitly still works.
 */
export const OPT_IN_ONLY_SCOPES: readonly Scope[] = ['files:content', 'memories:read', 'plugins:use'];
...
export const SCOPE_INFO: Record<Scope, ScopeInfo> = {
  ...
  'memories:read': {
    label: 'Browse photo libraries',
    description: 'Search connected photo libraries and read capture metadata to attach photos to journeys',
    group: 'Photo libraries',
  },
  'files:read': {
